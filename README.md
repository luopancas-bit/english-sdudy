...
- Qwerty 键入训练；章节词汇已接入释义、固定课程音频、完整拼写和例句填空四维正式考核
- 单词四维综合达到 80 分才显示本次达标，未通过不锁章节、不阻止学习
- 单词间隔复习已接入 `1、3、7、14、30` 天调度、失败退阶、到期复核和证据记录
- 单词记忆和生词本共用版本化音标库，显示美式、英式 IPA，并支持安全暂存和导入 MDX/MDD 词典包
- 40 课均已建立固定四维题库和美音单词考核资源；前三课保留实机验证基线，后续课程继续抽样核对发音质量
...
词典数据结构、MDX/MDD 暂存发布流程、CSS/JS 安全处理和版权隔离规则见
[词典与音标库](docs/dictionary-pronunciation-library.md)。
