...
- 听力和听写按私有强制对齐时间播放对应句子，不向客户端泄露原文答案。
- 正式口语题必须有归属当前用户和题目的录音记录；当前文本准确度评分与原始录音证据分开保存，未接入语音模型前不宣称提供发音质量评分。
- 修改密码必须再次验证当前密码，成功后撤销其他设备会话；设备管理接口只返回显示所需信息，不返回会话令牌或令牌哈希。
